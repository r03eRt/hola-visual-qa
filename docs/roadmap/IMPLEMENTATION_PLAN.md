...
- `feature/playwright-placement-event-bridge` (#85) — Playwright event bridge (`installPlacementEventBridge`) feeding the request/render evaluators, plus `assertPlacementResolved`; ad request/render stages now run in the browser from page-emitted debug events (no raw payload capture). No-op unless a placement declares `events`.
