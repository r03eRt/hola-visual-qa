...
test.describe('createAdStateAdapter', () => {
  test('defaults flagName to "__ADS_ENABLED__" when config does not set one', () => {
    ...
    const config = validConfig({ adapters: { consent: {}, ads: { strategy: 'init-script' }, country: {} } });
  ...
  test('uses config.adapters.ads.flagName override when present', () => {
    ...
    const config = validConfig({ adapters: { consent: {}, ads: { strategy: 'init-script', flagName: 'my_ads_flag' }, country: {} } });
